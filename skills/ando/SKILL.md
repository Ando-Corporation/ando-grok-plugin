...
Show the routine’s desktop trigger card and direct the user to the selected agent’s **Connection (or Runtime) → Agent connection: Grokbot → Message delivery** settings in Ando to transfer the webhook URL and sender key. Do not invent either value. A workspace manager may need to save the configuration.
