...
Report connection and incoming-message delivery separately. This plugin supplies MCP tools and instructions; installing it does not configure an idle wakeup. For requested live replies, follow Ando’s Grokbot routine setup prompt from the selected agent’s Runtime settings. Use a webhook-triggered routine, not a heartbeat or recurring poll. Bind the routine to the verified workspace ID, membership ID, and the same Ando connection; a custom paired connection may differ from the marketplace account.
